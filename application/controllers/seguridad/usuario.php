...
class Usuario extends CI_Controller {
    ...
    public function buscarusuario() {
        ...
        if ($tipousuario >= 3) {
            if ($resultdbd = $this->usuario_model->cargarusuariodiv2($username['cedula'], $username['nacionalidad'])) {
            ...
            if ($resultdbd = $this->usuario_model->cargarusuario2($username['cedula'], $username['nacionalidad'])) {
                $this->output->set_content_type('application/json');
                $this->output->set_output(json_encode(array(
                    "success" => True,
                    'data' => $resultdbd)));
    ...
    public function registrarusuario() {
        ...
        if (isset($add)) {
            ...
            foreach ($records as $record) {
                ...
                $nacionalidad = $record->nacionalidad;
                $usuario = $record->rif;
                ($resultado = $this->usuario_model->existepersona($cedula, $nacionalidad));
                if ($resultado == '') {
                    $result2 = $this->usuario_model->insertpersona($datapersona);
                    $result3 = $this->usuario_model->insertusuario($datausuario);
                    $result = $this->usuario_model->updateusuariolinea($usuario);
    ...
    Registro LAE  le da la más cordial bienvenida al Registro de Empresas por Actividad Económica del Estado Lara. Le notificamos que su Usuario y Clave se realizó exitosamente:
    </p>
    <p><b>Usuario:' . $usuario . '</b></p>
    <p><b>Clave:' . $cad . '</b></p>
    <p>IMPORTANTE: Recuerde que estos datos son confidenciales e intransferibles.</p>
    Esta es una cuenta de correo no monitoreada. Por favor, no responda ni reenvie mensajes a esta cuenta
    <p>NOTA:Esta es una cuenta de correo no monitoreada. Por favor, no responda ni reenvie mensajes a esta cuenta<b>Sistema de Registro de Empresas Manufatureras</b></p>
